...
const handleDelete = (e) => {
    e.preventDefault();
    const deleteButton = e.target.closest('[data-kt-roles-table-delete]');
    if (!deleteButton) return;

    const roleId = deleteButton.getAttribute('data-kt-roles-table-delete');
    const roleName = deleteButton.getAttribute('data-kt-roles-table-name');
    ...
    Swal.fire({
        ...
        if (result.value) {
            // Send DELETE request
            fetch(`/admin/staff/roles/destroy/${roleId}`, {
                method: 'DELETE',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    Swal.fire({
                        text: data.message || "Rol silinirken bir hata oluştu!",
                        icon: "error",
                        buttonsStyling: false,
                        confirmButtonText: "Tamam",
                        customClass: {
            ...
            .catch(error => {
                Swal.fire({
                    text: "Rol silinirken bir hata oluştu!",
                    icon: "error",
                    buttonsStyling: false,
                    confirmButtonText: "Tamam",
                    customClass: {
