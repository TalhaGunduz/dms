...
@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Yeni Günlük Menü Oluştur</h3>
        </div>
        <div class="card-body">
            @if($errors->any())
            ...
            <form action="{{ route('admin.menu.daily.store') }}" method="POST">
                @csrf
                <div class="row mb-5">
                    <div class="col-md-6">
                        <label class="form-label required">Tarih</label>
                        <input type="date" name="menu_date" class="form-control @error('menu_date') is-invalid @enderror" value="{{ old('menu_date') }}" required>
                        @error('menu_date')
