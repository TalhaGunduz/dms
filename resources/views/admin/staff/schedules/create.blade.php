...
@section('title', 'Yeni Çalışma Programı')

@section('content')
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Yeni Çalışma Programı</h3>
    </div>
    <div class="card-body">
        <form id="kt_schedule_form" action="{{ route('admin.staff.schedules.store') }}" method="POST">
            @csrf
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('day_of_week') is-invalid @enderror" id="day_of_week" name="day_of_week" required>
                    ...
                    <option value="weekdays" {{ old('day_of_week') === 'weekdays' ? 'selected' : '' }}>Hafta İçi Her Gün</option>
                    <option value="monday" {{ old('day_of_week') === 'monday' ? 'selected' : '' }}>Pazartesi</option>
                    <option value="tuesday" {{ old('day_of_week') === 'tuesday' ? 'selected' : '' }}>Salı</option>
                    <option value="wednesday" {{ old('day_of_week') === 'wednesday' ? 'selected' : '' }}>Çarşamba</option>
                    <option value="thursday" {{ old('day_of_week') === 'thursday' ? 'selected' : '' }}>Perşembe</option>
                    <option value="friday" {{ old('day_of_week') === 'friday' ? 'selected' : '' }}>Cuma</option>
                    <option value="saturday" {{ old('day_of_week') === 'saturday' ? 'selected' : '' }}>Cumartesi</option>
                    <option value="sunday" {{ old('day_of_week') === 'sunday' ? 'selected' : '' }}>Pazar</option>
                </select>
                @error('day_of_week')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="mb-3">
                ...
                <input type="time" class="form-control @error('break_start') is-invalid @enderror" id="break_start" name="break_start" value="{{ old('break_start') }}">
                @error('break_start')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="time" class="form-control @error('break_end') is-invalid @enderror" id="break_end" name="break_end" value="{{ old('break_end') }}">
                @error('break_end')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
